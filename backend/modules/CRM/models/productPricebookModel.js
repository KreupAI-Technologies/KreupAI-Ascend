...
productPriceBookSchema.pre('save', async function (next) {
    ...
    if (!this.serialNo) {
        ...
        if (this.serialNo > count + 1) {
            return next(
                new Error('Serial Number must be sequential within the same product and price book')
            );
        }
    }
...
const ProductPriceBook = mongoose.model('ProductPriceBook', productPriceBookSchema);
