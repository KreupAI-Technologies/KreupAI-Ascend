...
import SalesOrderLine from "../models/salesOrderLineModel.js";
  ...
  try {
    ...
    if (error.code === 11000) {
      return res
        .status(400)
        .json({
          message: "Serial Number must be unique within the sales order",
        });
    }
...
export const getSalesOrderLinesBySaleOrderId = async (req, res) => {
  try {
...
export const deleteSalesOrderLine = async (req, res) => {
  try {
    const salesOrderLine = await SalesOrderLine.findByIdAndDelete(
      req.params.id
    );
